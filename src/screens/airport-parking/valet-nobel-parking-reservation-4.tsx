...
import ValetRecheckPhonePopup, {
  ValetRecheckPhoneRefs,
} from '~components/valet-parking-reservation/valet-recheck-phone-popup';

const ValetNobelParkingReservation4 = memo(
  (props: RootStackScreenProps<'ValetNobelParkingReservation4'>) => {
    ...
    const valetRecheckPhoneRef = useRef<ValetRecheckPhoneRefs>(null);
    ...
    const handleReservationWithCreditCard = () => {
      Spinner.show();
    ...
    const handleConfirm = () => {
      ...
      if (paymentMethod === PAYMENT_METHOD.CELL_PHONE) {
        valetRecheckPhoneRef?.current?.show();
      }
    };

    return (
      <FixedContainer>
        <CustomHeader text="4단계. 발렛주차 예약결제" />
        <ScrollView>
        ...
        <ValetParkingReservationPopup
          ...
          onConfirm={handleReservationWithCreditCard}
        />
        ...
        {/* Valet recheck phone */}
        <ValetRecheckPhonePopup
          garageId={parkingLot?.id}
          garageName={parkingLot?.garageName}
...
export default ValetNobelParkingReservation4;
